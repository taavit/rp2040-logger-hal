...
use super::configuration::{AccelerationFullScale, Configuration};
    ...
    I2C: WriteRead + Write + Read,
{
    ...
    pub fn read_measurements(&mut self) -> Result<Measurements, ()> {
        ...
        self.i2c
            ...
            .map_err(|_| ())?;
        acc_x = (f64::from(i16::from_le_bytes(buffer)) * self.acc_multiplier) / 32678.0;
        self.i2c
            .write_read(self.address, &[Register::OutYLA as u8 | 0x80], &mut buffer)
            .map_err(|_| ())?;
        acc_y = (f64::from(i16::from_le_bytes(buffer)) * self.acc_multiplier) / 32678.0;
        self.i2c
            .write_read(self.address, &[Register::OutZLA as u8 | 0x80], &mut buffer)
            .map_err(|_| ())?;
        acc_z = (f64::from(i16::from_le_bytes(buffer)) * self.acc_multiplier) / 32678.0;
